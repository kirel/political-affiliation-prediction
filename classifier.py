...
class Classifier:
    ...
    def __init__(self,folder='manifestoproject',train=False):
        '''
        ...
        self.clf = cPickle.load(open(self.folder+'/classifier.pickle'))
    ...
    def train(self,folds = 2):
        ...
        INPUT
        folder  the folder to store the model file and training data (as csv files)
        folds   number of cross-validation folds for model selection 

        '''
        try:
            # load the data
            data,labels = get_raw_text(folder=self.folder)
        except:
        ...
        cPickle.dump(gs_clf.best_estimator_,open(self.folder+'/classifier.pickle','wb'),-1)
